...
class Test_SignIn:
    ...
    def test_with_good_email_password_full_automation(self, driver, good_email, good_password):
        ...
        assert page.current_url() == ACCOUNT_PAGE , "Не удалось залогиниться в существующий аккаунт , функция автологин"
    ...
    def test_with_bad_email_password_full_automation(self, driver, fake_email, fake_password):
        ...
        assert page.current_url() != ACCOUNT_PAGE, "удалось залогиниться в несуществующий аккаунт"
        assert page.error_signin_msg().text == ERROR_SIGNIN_MSG, "неправильное сообщение об ошибке при попытке логина с фейковыми данными, функция автологин"
